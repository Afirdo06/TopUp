...
<body>
    <div class="container">
        ...
        <form action="<?= site_url('/admin/updatePackage/'.$package['id']) ?>" method="POST">
    <?= csrf_field() ?>
    ...
    <div class="mb-3">
        ...
        <select class="form-control" id="currency_type" name="currency_type" required>
            <option value="IDR" <?= old('currency_type', $package['currency_type']) == 'IDR' ? 'selected' : '' ?>>IDR</option>
            <option value="USD" <?= old('currency_type', $package['currency_type']) == 'USD' ? 'selected' : '' ?>>USD</option>
        </select>
    </div>
